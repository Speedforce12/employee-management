...
import { getUserData, toggleUpdateForm } from "../redux/reducer";
        ...
        {data?.map((obj, i) => (
          <tr className='bg-gray-50 text-center' key={i}>
            <td className='px-16 py-2 flex flex-row items-center relative'>
              ...
              />
              <span className='text-center ml-2 font-semibold text-sm lg:text-base lg:whitespace-nowrap'>
                {obj.name}
              </span>
            </td>
            ...
            <td className='px-16 py-2 whitespace-nowrap'>
              <span>{obj.date}</span>
            </td>
            <td className='px-16 py-2'>
              <button className='cursor-point'>
                <span
                  className={`rounded-full px-4 py-1 text-white ${
                    obj.status === "Active" ? "bg-green-600" : "bg-rose-500"
                  } `}>
                  {obj.status ? obj.status : " "}
                </span>
              </button>
            </td>
            <td className='px-16 py-2 space-x-4 flex'>
              <button type='button'>
                <BiEdit
                  size={25}
                  color={"rgb(34,197,94)"}

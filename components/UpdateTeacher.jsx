...
export default function userDetail() {
  ...
  const updateMutation = useMutation((updated)=>updateEmployee(employeeId, updated), {
    onSuccess: () => {
  ...
  // render update form once user data has been fetch successfully from backend
    if (data) {
    return <UpdateTeacher user={data} onSubmit={onSubmit} />;
  }
  return (
...
function UpdateTeacher({ user, onSubmit }) {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='flex items-center space-x-4 justify-between'>
          <button
            ...
            type='submit'
            className='gap-x-2 text-white items-center inline-flex bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-yellow-400 dark:hover:bg-yellow-600 dark:focus:ring-yellow-700'>
            Update Teacher
          </button>
          <button
